...
function propHandler(documentation, path) {
    return __awaiter(this, void 0, void 0, function () {
        var functionalPath, functionalValue, componentCommentedPath, classDeclaration, docBlock, jsDoc, displayNamesTags, displayName, tagsAsObject, examples;
        return __generator(this, function (_a) {
            // deal with functional flag
            if (bt.isObjectExpression(path.node)) {
                functionalPath = getProperties_1.default(path, 'functional');
                if (functionalPath.length) {
                    functionalValue = functionalPath[0].get('value').node;
                    if (bt.isBooleanLiteral(functionalValue)) {
                        documentation.set('functional', functionalValue.value);
                    }
                }
            }
            componentCommentedPath = path.parentPath;
            ...
            if (bt.isCallExpression(componentCommentedPath.node)) {
                ...
                classDeclaration = componentCommentedPath.get('declaration');
                if (bt.isClassDeclaration(classDeclaration.node)) {
                    componentCommentedPath = classDeclaration;
                }
            }
            docBlock = getDocblock_1.default(componentCommentedPath);
            // if no prop return
            if (!docBlock || !docBlock.length) {
                return [2 /*return*/];
            }
            jsDoc = getDoclets_1.default(docBlock);
            documentation.set('description', jsDoc.description);
            if (jsDoc.tags) {
                displayNamesTags = jsDoc.tags.filter(function (t) { return t.title === 'displayName'; });
                if (displayNamesTags.length) {
                    displayName = displayNamesTags[0];
                    documentation.set('displayName', displayName.content);
                }
                tagsAsObject = transformTagsIntoObject_1.default(jsDoc.tags.filter(function (t) { return t.title !== 'example' && t.title !== 'displayName'; }) || []);
                examples = jsDoc.tags.filter(function (t) { return t.title === 'example'; });
                if (examples.length) {
